...
from .langchain import invoke_answer
import buttons.cardAction as btn
...
class EchoBot(ActivityHandler):
        ...
        for member in members_added:
            if member.id != turn_context.activity.recipient.id:
                await turn_context.send_activity(
                    "Hola! Soy un futuro bot de Teams hecho con Langchain"
                )
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        #print(conversation_data.chat_history)

        response = invoke_answer({"input": turn_context.activity.text, "messages": conversation_data.chat_history})
        conversation_data.chat_history.append({"role": "user", "content": turn_context.activity.text})
        conversation_data.chat_history.append({"role": "assistant", "content": response})

        ideas_list = ["Idea 1", "Idea 2", "Idea 3", "Idea 4", "Idea 5", "Idea 6"]
        reply = MessageFactory.text(f"{response}")
        reply.suggested_actions = SuggestedActions(
        actions= btn.get_action_cards(ideas_list)
